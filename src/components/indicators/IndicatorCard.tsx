...
export function IndicatorCard({ item, onCopy }: IndicatorCardProps) {
  return (
    <article className="rounded-md border border-border/60 bg-background p-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-xl font-semibold tracking-tight text-foreground">{item.valor}</p>
          <p className="mt-1 text-[11px] text-muted-foreground">Corte: {item.fechaCorte}</p>
        </div>
        <button
          type="button"
          onClick={() => onCopy(item)}
